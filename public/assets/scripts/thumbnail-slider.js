$(document).ready(function () {
	...
	$('.product-sidebar__size-link').click(function (e) {
		...
		$('.product-sidebar__size-link').not(this).removeClass('product-sidebar__size-link--active')
		$(this).addClass('product-sidebar__size-link--active')

		$('.product-sidebar__price-item').removeClass('product-sidebar__price-item--active')
		$(`[data-id="${$(this).attr('href')}"]`).addClass('product-sidebar__price-item--active')
	})
...
const notifyModal = document.querySelector('.product-notify')

addBtn.addEventListener('click', () => {
	notifyModal.classList.add('product-notify--active')

	setTimeout(() => {
		notifyModal.classList.remove('product-notify--active')
	}, 2000)
})
